...
- On a related note to the above, it can also be derived that we would never be able to generate certain
  frequencies. For example, 72 MHz would never be possible to generate in the EOSS3 system.
...
- Another consequence of all the domain clocks being generated from HSOSC is that anytime one of the domain
...
- Generic calculation method if any value of clock is **exactly** possible with a specific HSOSC value:
  ...
  So, if CLOCK_FREQ / HSOSC ratio can be reduced as a fraction, with numerator < 4096 and div < 512
...
To understand the clocks which we generally need to look at we will take an "example design" approach
which maybe easier to digest in a step-by-step manner, rather than taking the system as a whole.
...
:code:`TODO: check if we can make such calculations possible automatically, maybe with a script.`
...
Side Note: we skipped using all of the constaints (LPSD 512000 Hz and REFCLK 32768 Hz) as it has already
