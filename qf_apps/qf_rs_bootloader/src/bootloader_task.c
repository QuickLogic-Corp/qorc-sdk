...
#define BL_MODE_BOOTLOADER   (0x55555555)
...
uint32_t  gbootLoaderMode[16]={0};
extern void read_flash(unsigned char *start_address, int length, unsigned char *destination);

uint32_t bl_mode_get(void)
{
    read_flash((unsigned char *)FLASH_BOOTLOADER_META_ADDRESS, sizeof(gbootLoaderMode), (unsigned char *)gbootLoaderMode); // read bootloader mode
    return gbootLoaderMode[2];
}
...
static void BLTaskHandler(void *pvParameters)
{
    ...
    TickType_t startTicks, endTicks;
    uint32_t blMode;
	...
	while(1)
	{
      ...
      blMode = bl_mode_get();
      if(user_button_pressed || (blMode == BL_MODE_BOOTLOADER))
      {
      ...
      wait_time_msec = endTicks - startTicks;
