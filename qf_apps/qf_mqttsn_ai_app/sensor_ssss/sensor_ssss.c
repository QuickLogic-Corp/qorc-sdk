...
#if (SENSOR_SSSS_RECOG_ENABLED) && (S3AI_FIRMWARE_IS_RECOGNITION)
...
#if (SENSOR_SSSS_LIVESTREAM_ENABLED) && (S3AI_FIRMWARE_IS_COLLECTION)
...
sensor_ssss_config_t sensor_ssss_config;
     ...
     )
{
    ...
    if (sensor_ssss_config.enabled == true)
    {
      ...
      sdi.sensor_id  = SENSOR_SSSS_ID;
      sdi.rate_hz    = sensor_ssss_config.rate_hz;
      ...
      mqttsn_publish_sensor_data( &sensor_ssss_config, &sdi );
    }
    *pRet = NULL;
    return;
}
