...
int load_appfpga(void)
{
  ...
  {
    dbg_str("APPFPGA Image size exceeded bootable size \n");
    return BL_ERROR;
  }
  ...
  load_fpga_with_mem_init(fpga_bitstream_size, fpga_bitstream_ptr, fpga_meminit_size, fpga_meminit_size);
  fpga_iomux_init(fpga_iomux_size, fpga_iomux_ptr);

  dbg_str("APPFPGA Programmed\n");
