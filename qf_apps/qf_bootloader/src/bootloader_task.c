...
#define M4APP_SELECTION_GPIO_NUM  (2) //PAD 11, (TBD) placeholder
#define M4APP_SELECTION_GPIO_NUM2 (3) //PAD 12?, (TBD) placeholder
...
#define METADATA_M4APP2_IS_FLASHED          (0x8)
#define METADATA_M4APP3_IS_FLASHED          (0x10)
#define METADATA_M4APP4_IS_FLASHED          (0x20)
#define METADATA_VALID_IMAGE_IS_FLASHED     (METADATA_APPFPGA_IS_FLASHED|METADATA_M4APP_IS_FLASHED) // ffe not supported yet

uint8_t g_m4app_selection_gpio_value[2] ;
...
typedef struct st_active_image_info {
	uint32_t image_meta_address;
	uint32_t image_meta_size;
	uint8_t  is_flashed;
} active_image_info_t;

active_image_info_t active_image_info[]=
{
   { FLASH_APPFPGA_META_ADDRESS, FLASH_APPFPGA_META_SIZE, METADATA_APPFPGA_IS_FLASHED },
   { FLASH_APPFFE_META_ADDRESS,  FLASH_APPFFE_META_SIZE,  METADATA_APPFFE_IS_FLASHED },
   { FLASH_APP_META_ADDRESS,     FLASH_APP_META_SIZE,     METADATA_M4APP_IS_FLASHED },
   { FLASH_APP2_META_ADDRESS,    FLASH_APP2_META_SIZE,    METADATA_M4APP2_IS_FLASHED },
   { FLASH_APP3_META_ADDRESS,    FLASH_APP3_META_SIZE,    METADATA_M4APP3_IS_FLASHED },
   { FLASH_APP4_META_ADDRESS,    FLASH_APP4_META_SIZE,    METADATA_M4APP4_IS_FLASHED },
};
int check_active_images()
{
    ...
    int num_images = sizeof(active_image_info)/sizeof(active_image_info[0]);
    for (int k = 0; k < num_images; k++)
    {
    	uint32_t meta_address = active_image_info[k].image_meta_address;
    	uint32_t meta_size    = active_image_info[k].image_meta_size;
    	uint8_t  is_flashed   = active_image_info[k].is_flashed;
        // read the fpga metadata
        bufPtr = (unsigned char *)image_metadata;
        read_flash((unsigned char *)meta_address, meta_size, bufPtr);
