...
QL_Status BMI160_Accel2_Probe_In_M4();
...
static QL_Status Accel_probe(struct QL_SDF_SensorDrv *s)
{
	...
	if(QL_STATUS_OK != BMI160_Accel2_Probe_In_M4())
	{
		QL_TRACE_SENSOR_ERROR("BMI160_Accel2_Probe_In_M4() failed for SAL sensor with ID = %d\n",accel_drv_priv.drvData.id);
	...
	QL_TRACE_SENSOR_DEBUG("BMI160_Accel2_Probe_In_M4() OK for SAL sensor with ID = %d\n",accel_drv_priv.drvData.id);
