...
#define CONFIG_MAX_FFE_PCG_INSTANCE 2
...
typedef struct
{
	...
	/* private stuff below */
	unsigned int  ffe_pcg_id;											/* FFE Side Sensor ID */
	QL_PCG_Ffe_Dev devInstances[CONFIG_MAX_FFE_PCG_INSTANCE];		/* instance specific data */
}
QL_PCG_Ffe_Drv;


static QL_PCG_Ffe_Drv pcg_drv_priv;

// private function declarations
QL_Status PCG_probe(struct QL_SDF_SensorDrv *s);
QL_Status PCG_Open(struct QL_SDF_SensorDrv *s, QL_SDF_SensorDrvInstanceHandle_t *instance_handle);
QL_Status PCG_Close(QL_SDF_SensorDrvInstanceHandle_t instance_handle);
QL_Status PCG_read(QL_SDF_SensorDrvInstanceHandle_t instance_handle, void *buf, int size);
QL_Status PCG_ioctl(QL_SDF_SensorDrvInstanceHandle_t instance_handle, unsigned int cmd, void *arg);
QL_Status PCG_suspend(QL_SDF_SensorDrvInstanceHandle_t instance_handle);
QL_Status PCG_resume(QL_SDF_SensorDrvInstanceHandle_t instance_handle);
//QL_Status PCG_IsArgValid(unsigned int cmd, void* arg);



static QL_Status PCG_probe(struct QL_SDF_SensorDrv *s)
{
	unsigned int index;

	QL_ASSERT(s != NULL);
	QL_ASSERT(s == (struct QL_SDF_SensorDrv *)&pcg_drv_priv); // we should get back the same instance that we provided during registration
	...
	if (QL_STATUS_OK != QL_FFE_Sensor_Probe(pcg_drv_priv.ffe_pcg_id))
	{
		QL_TRACE_SENSOR_ERROR("QL_FFE_Sensor_Probe() failed for FFE sensor with ID = %d\n",pcg_drv_priv.ffe_pcg_id);

		/* probe failed, update the state of each of the instances to not ready, SENSOR_STATE_INVALID */
		for ( index = 0 ; index < CONFIG_MAX_FFE_PCG_INSTANCE; index++ )
		{
			pcg_drv_priv.devInstances[index].state = SENSOR_STATE_INVALID;
		}
	...
	for ( index = 0 ; index < CONFIG_MAX_FFE_PCG_INSTANCE; index++ )
	{
		pcg_drv_priv.devInstances[index].state = SENSOR_STATE_PROBED;
	}
...
static QL_Status PCG_Open(struct QL_SDF_SensorDrv *s, QL_SDF_SensorDrvInstanceHandle_t *instance_handle)
{
	unsigned int index ;

	*instance_handle = (QL_SDF_SensorDrvInstanceHandle_t)0xFFFF;
	QL_ASSERT(s != NULL);
	QL_ASSERT(s == (struct QL_SDF_SensorDrv *) &pcg_drv_priv); // we should get back the same  that we provided during registration

	for ( index = 0 ; index < CONFIG_MAX_FFE_PCG_INSTANCE; index++ )
	{
//		printf("instance index: %d, state: %d\n",index, pcg_drv_priv.devInstances[index].state);
		/* if we have an instance slot with state == SENSOR_STATE_PROBED, this is ready to use */
		if (SENSOR_STATE_PROBED == pcg_drv_priv.devInstances[index].state)
		{
			/* move this instance's state to SENSOR_STATE_OPENED to mark that this slot is now in use */
			pcg_drv_priv.devInstances[index].state = SENSOR_STATE_OPENED;
...
static QL_Status PCG_Close(QL_SDF_SensorDrvInstanceHandle_t instance_handle)
{
	/* KK: check validity of handle. This is probably not needed and can be removed */
	QL_ASSERT((unsigned int) instance_handle < CONFIG_MAX_FFE_PCG_INSTANCE);

	/* move the state of this instance back to ready to use, SENSOR_STATE_PROBED */
	pcg_drv_priv.devInstances[(unsigned int)instance_handle].state = SENSOR_STATE_PROBED;
...
static QL_Status PCG_read(QL_SDF_SensorDrvInstanceHandle_t instance_handle, void *buf, int size)
{
...
#if 0 // Not used
static QL_Status PCG_IsArgValid(unsigned int cmd, void* arg)
{
...
static QL_Status PCG_ioctl(QL_SDF_SensorDrvInstanceHandle_t instance_handle, unsigned int cmd, void *arg)
{
	unsigned int sensorid;
	QL_Status status = QL_STATUS_OK;

	sensorid = pcg_drv_priv.ffe_pcg_id;
	QL_ASSERT( (unsigned int)instance_handle < CONFIG_MAX_FFE_PCG_INSTANCE);

//	printf("sensorid: %u, cmd: %u\r\n", sensorid, cmd);

	/* check if the instance is in open state first */
	if (SENSOR_STATE_OPENED != pcg_drv_priv.devInstances[(unsigned int) instance_handle].state)
	{
		return QL_STATUS_ERROR;
	}
...
//	if(QL_STATUS_OK != PCG_IsArgValid(cmd, arg))
//	{
...
//	}
	...
	switch (cmd)
	{
		...
		{
			pcg_drv_priv.drvData.sensor_callback = (QL_SensorEventCallback_t)arg;
		}
		break;

		case QL_SAL_IOCTL_GET_CALLBACK:
		{
			*(QL_SensorEventCallback_t *)arg = pcg_drv_priv.drvData.sensor_callback;
		}
		break;
...
static QL_Status PCG_suspend(QL_SDF_SensorDrvInstanceHandle_t instance_handle)
{
...
static QL_Status PCG_resume(QL_SDF_SensorDrvInstanceHandle_t instance_handle)
{
...
__PLATFORM_INIT__ QL_Status PCG_init()
{
	unsigned int index;

	pcg_drv_priv.drvData.id = QL_SAL_SENSOR_ID_PCG;
	pcg_drv_priv.drvData.type = QL_SDF_SENSOR_TYPE_COMPOSITE;		/* composite/virtual sensor */
	//pcg_drv_priv.drvData.max_sample_rate = 0; 				// KK: this is not needed here at all, can be removed ?
	//pcg_drv_priv.drvData.name = "PCG"; 					// KK: this is not used anywhere, can be removed ?

	pcg_drv_priv.drvData.ops.open = PCG_Open;
	pcg_drv_priv.drvData.ops.close = PCG_Close;
	pcg_drv_priv.drvData.ops.read = PCG_read;
	pcg_drv_priv.drvData.ops.probe = PCG_probe;
	pcg_drv_priv.drvData.ops.ioctl = PCG_ioctl;
	pcg_drv_priv.drvData.ops.suspend = PCG_suspend;
	pcg_drv_priv.drvData.ops.resume = PCG_resume;

	/* the FFE side ID is common for all instances, it is a sensor/driver level detail */
	pcg_drv_priv.ffe_pcg_id = QL_FFE_SENSOR_ID_PCG;

	/* init framework level probe status to default, failed state - updated when probe is called */
	pcg_drv_priv.drvData.probe_status = SENSOR_PROBE_STATUS_FAILED;
	...
	pcg_drv_priv.drvData.sensor_callback = NULL;

	/* Do any platform related pin mux / I2C setting */

	/* register with SDF */
	if (QL_STATUS_OK != QL_SDF_SensorDrvRegister((struct QL_SDF_SensorDrv *) &pcg_drv_priv))
	{
