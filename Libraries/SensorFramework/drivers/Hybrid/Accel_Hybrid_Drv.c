...
QL_Status BMI160_Accel_Probe_In_M4();
...
static QL_Status Accel_probe(struct QL_SDF_SensorDrv *s)
{
	 ...
	 *
	 */
	if(QL_STATUS_OK != BMI160_Accel_Probe_In_M4())
	{
		QL_TRACE_SENSOR_ERROR("BMI160_Accel_Probe_In_M4() failed for SAL sensor with ID = %d\n",accel_drv_priv.drvData.id);
...
__PLATFORM_INIT__ QL_Status Accel_init()
{
	unsigned int index;

    accel_drv_priv.name = _ascii_accel;
	...
	accel_drv_priv.drvData.name = _ascii_accel; 					// KK: this is not used anywhere, can be removed ?
