...
QL_Status BMI160_Gyro_Probe_In_M4();
...
QL_Status Gyro_probe(struct QL_SDF_SensorDrv *s);
QL_Status Gyro_Open(struct QL_SDF_SensorDrv *s, QL_SDF_SensorDrvInstanceHandle_t *instance_handle);
QL_Status Gyro_Close(QL_SDF_SensorDrvInstanceHandle_t instance_handle);
QL_Status Gyro_read(QL_SDF_SensorDrvInstanceHandle_t instance_handle, void *buf, int size);
QL_Status Gyro_ioctl(QL_SDF_SensorDrvInstanceHandle_t instance_handle, unsigned int cmd, void *arg);
QL_Status Gyro_suspend(QL_SDF_SensorDrvInstanceHandle_t instance_handle);
QL_Status Gyro_resume(QL_SDF_SensorDrvInstanceHandle_t instance_handle);
...
static QL_Status Gyro_probe(struct QL_SDF_SensorDrv *s)
{
	...
	if(QL_STATUS_OK != BMI160_Gyro_Probe_In_M4())
	{
		QL_TRACE_SENSOR_ERROR("BMI160_Gyro_Probe_In_M4() failed for SAL sensor with ID = %d\n",gyro_drv_priv.drvData.id);
	...
	QL_TRACE_SENSOR_DEBUG("BMI160_Gyro_Probe_In_M4() OK for SAL sensor with ID = %d\n",gyro_drv_priv.drvData.id);
...
__PLATFORM_INIT__ QL_Status Gyro_init()
{
	...
	//gyro_drv_priv.drvData.name = "Gyro"; 					// KK: this is not used anywhere, can be removed ?
...
#define BMI160_ACC_CONF_REG        	0x40 //Gyro configuration
#define BMI160_ACC_PMU_RANGE_REG   	0x41 //Gyro Range setup
