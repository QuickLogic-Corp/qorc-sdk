...
repository using ```$ git clone https://github.com/QuickLogic-Corp/qorc-sdk```.

Refer [Helloworldsw][] to build and run example application project.
...
* Toolchain
    - [ARM GNU GCC toolchain] Version 7.2.1 or later
      ...
      $ sudo apt-get update     
      $ sudo apt-get install gcc-arm-embedded/ppa  

      ```
* Flash programmer toolchain: [TinyFPGA programmer]
    - To install clone the repository and install the dependancy
    ```
    $ git clone https://github.com/timsaxe/TinyFPGA-Programmer-Application.git
...
To build and run the qf_helloworldsw projects
  ...
  9. Type ‘help’ and you should see:  
    ![qf_helloworld CLI Help][qf_helloworldsw-cli-help]  
     ...
     Which lists what is in the top-level CLI menu:
        i.  diag is a user defined sub-menu
        ii. the others are there by default
...
## Example \#1b: M4 only – modify qf_helloworldsw
  ...
  4. Reset QuickFeather board and press ‘user button’ while blue LED is flashing
       a. Should switch to mode where green LED is breathing
       b. If green LED not breathing, press reset again and ‘user button’ within 5 seconds of releasing reset
  ...
  6. After programming has completed, reset the QuickFeather board and do not press the user button
    a. Blue LED should flash for 5 sec and then load the m4app and run it
...
## Example #2a: FPGA only – qf_helloworldhw
  1. $ cd qorcsdk/qf_apps/qf_helloworldhw/GCC_projects
  2. $ make
  3. Reset QuickFeather board and press ‘user button’ while blue LED is flashing
    a. Should switch to mode where green LED is breathing
    b. If green LED not breathing, press reset again and ‘user button’ within 5 seconds of releasing reset
  ...
  5. After programming has completed, reset the QuickFeather board and do not press the user button
    a. Blue LED should flash for 5 sec and then load the m4app which will load the FPGA and run that
  ...
  8. How does this work:
    a. If you look at main.c you will find the following code fragment:
    ```c
    ...
    b. The array, axFPGABitStream, is located in fpga/rtl/helloworldfpga_bit.h which is generated from the Verilog by ql_symbiflow
    c. What the ```load_fpga()```  does is take the bitstream from array axFPGABitSTream and program the FPGA
    d. What the ```S3x_Clk_Enable(S3X_FB_21_CLK)``` does is to enable CLK 21 which is one of two clocks from the M4 that drive the FPGA
    e. The frequency of the clock is specified in src/s3xd_pwrcfg.c to be 48MHz
       ```
       [CLK_C21] = {
        .name = "C21",
        .clkd_id = CLK_C21,
...
## Example \#2b: FPGA only – modify qf_helloworldhw
  1. Using the editor of your choice, edit qf_apps/qf_helloworldhw/fpga/rtl/helloworldfpga.v
    a. Change the terminal counter from 4000000 to 2000000
    b. Change the assign greenled = led to assign redled = led
    c. Save the changes
  2. $ cd qf_apps/qf_helloworldhw/GCC_projects
  3. $ make
  4. Reset QuickFeather board and press ‘user button’ while blue LED is flashing
    a. Should switch to mode where green LED is breathing
    b. If green LED not breathing, press reset again and ‘user button’ within 5 seconds of releasing reset
  5. With green LED breathing, program the updated qf_helloworldhw app into QuickFeather:
    $ qfprog - -port /dev/ttyXX - -m4app output/bin/qf_helloworldhw.bin
  6. After programming has completed, reset the QuickFeather board and do not press the user button
    a. Blue LED should flash for 5 sec and then load the m4app which loads the new FPGA code and runs it
...
## Lesson \#3: Advanced fpga M4 + FPGA– qf_advancedfpga
  ...
  3. Connect to the QuickFeather UART
     a. Connect a serial-to-USB cable to the QuickFeather board
     b. Connect PuTTY to the serial port associated with the serial-to-USB cable
     c. Set baud rate to 115200
  4. Reset QuickFeather board and press ‘user button’ while blue LED is flashing
     a. Should switch to mode where green LED is breathing
     b. If green LED not breathing, press reset again and ‘user button’ within 5 seconds of releasing reset
  5. With green LED breathing, program m4 app into QuickFeather
     a. python tinyfpgaprogrammer - -port /dev/ttyXX - -m4app output/bin/qf_helloworldhw.bin
  6. After programming has completed, reset the QuickFeather board and do not press the user button
     a. Blue LED should flash for 5 sec and then load the m4app and run it
  7. The banner should be printed on PuTTY and then the CLI prompt
     a. Switch to the ledctlr sub-menu by entering ‘ledctlr’
     b. Set the color for timeslot 0 to blue by ‘color0 1’ – led should go blue
     c. Set the color for timeslot 1 to green by ‘color1 2’ – no visible change
     d. Set the color for timeslot 2 to red by ‘color2 4’ – no visible change
     e. Set the duration of timeslot 0 to 500ms by ‘duration0 500’
     f. Set the duration of timeslot 1 to 500ms by ‘duration1 500’
