...
int main(void)
{
    ...
    SOFTWARE_VERSION_STR = "qorc-sdk/qf_apps/qf_tfl_test2";
    ...
    dbg_str("\n\n");
    dbg_str( "##########################\n");
    dbg_str( "Quicklogic QuickFeather TensorFlow Lite\n");
    dbg_str( "SW Version: ");
	...
	dbg_str("calling tfl::setup()\n");
    setup(); 
	dbg_str("returned from tfl::setup()\n");
	loop();
	dbg_str("returned from tfl::loop()\n");
	dbg_str("infinite loop\n");
	dbg_str("infinite loop\n");
