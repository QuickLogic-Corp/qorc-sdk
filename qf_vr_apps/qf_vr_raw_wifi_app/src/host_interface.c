...
#define MULTICHANNEL_QUEUE_LENGTH (2*100) //must be able to handle messages while waiting for Host response
...
static void sendto_mch_queue(void)
{
  ...
  if(count > (MULTICHANNEL_QUEUE_LENGTH - 5)) {
...
//    printf("*%d",count);
  ...
  //send to queue
  ret = datablk_mgr_acquire(mch_db_manager, &pdatablk, 0);
  configASSERT(ret == 0);
...
//    return;
  ...
  {
    //assumption: mch_db_size is exact multple of 6 (240/2 = 120)
...
//    return;
  ...
  for(int i=0; i < data_size; i++)
  {
    ...
    mch_data_buf[mch_buf_wrcount++] = *ch1_data++; //channel 1
    mch_data_buf[mch_buf_wrcount++] = *ch2_data++; //channel 2
    mch_data_buf[mch_buf_wrcount++] = *ch3_data++; //channel 2
