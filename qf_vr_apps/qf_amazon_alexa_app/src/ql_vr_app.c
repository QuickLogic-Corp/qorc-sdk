...
#define THREAD_1_STACK_DEPTH (3*256) // = 3 Kbytes, since it is in words
...
#if (DSPC_AW == 0) //for 2 mic Solution this should not be enabled
//////////////////////////// STEREO2MONO PE BLOCK /////////////////////////////////

QueueHandle_t *audio_istereo2mono_outQs[1] = { &q_id_inQ_audio_processing};

/* ql vr processing element functions */
datablk_pe_funcs_t stereo2mono_funcs = {datablk_pe_config_stereo2mono, datablk_pe_process_stereo2mono, NULL, NULL, NULL } ;

outQ_processor_t audio_stereo2mono_outq_processor =
{
  .process_func = NULL,
  .p_dbm = &audioBuffDataBlkMgr,
  .in_pid = AUDIO_QL_PRE_PROC_PID,
  .outQ_num = 1,
  .outQ = audio_istereo2mono_outQs,
...
/* Audio PDM capture ISR */
#define AUDIO_OPUS_OUTQS_NUM (1)

//QueueHandle_t *audio_opus_outQs[AUDIO_OPUS_OUTQS_NUM] = { &q_id_inQ_audio_processing};

outQ_processor_t audio_ql_opus_outq_processor =
{
  .process_func = NULL,
  .p_dbm = &audioBuffDataBlkMgr,
  .in_pid = AUDIO_OPUS_PID,
  .outQ_num = 0,
  .outQ = NULL,
  .p_event_notifier = audio_ql_opus_event_handler
};
...
extern void opus_encodeBlock(QAI_DataBlock_t *pBlock);
...
#if (DSPC_AW == 0) //0=no pre-processing like ns or aec
