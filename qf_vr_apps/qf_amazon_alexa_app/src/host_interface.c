...
#define STREAM_KP_DETECT_ENABLE (1)
#define MAX_STREAM_KP_DETECTS  (10)
static int stream_kp_detect_enable = 0;
static int stream_kp_detect_count = 0;
...
#define D2H_MAX_DATA_SIZE   (4*960) //Max 8 blocks
    ...
    if(kbytes > 0) {
        ...
        if (p_ci->firstTime == 0){
            ...
            if (xSemaphoreTake(g_host_ready_lock, 2000) != pdTRUE){
                ...
                //configASSERT(0);
                dbg_str("[host interface] : Error unable to take lock to g_host_ready_lock\n");
            }
        }
        d2h_transmit_cmd(&d2h_evt_info);
    }
...
static void SendEventKPDetected(hif_channel_info_t* p_hif_channel_info, uint8_t evt)
{
  ...
  sprintf (host_sprintf_buf, "addr = 0x%X, size = %d\n", addr, size);
...
static void stop_audio_streaming(hif_channel_info_t *p_hif_channel_info)
{
  if (isStreamingOn())
  {
      //incase Data transmission does not start  but we receive PROCESS_OFF cmd
      //need to make sure the lock is taken so it is given in next READY_RECEIVE
      if (p_hif_channel_info->firstTime == 0){
            p_hif_channel_info->firstTime = 1;
            dbgtrace(__LINE__, 18, adbgtraceHIF, K_DBGTRACE_HIF, &idbgtraceHIF);
            if (xSemaphoreTake(g_host_ready_lock, 0) != pdTRUE){
                //configASSERT(0);
              dbg_str("--Error taking g_host_ready_lock--");
            }
            dbg_str("..LOCK_TAKEN..");
        }
...
sprintf (host_sprintf_buf, "Recd %d command\n",icommand);
dbg_str(host_sprintf_buf);
...
void transmit_done_callback(void *cookie, D2H_Pkt_Info tx_pkt_info)
{
    ...
    if (tx_pkt_info.cmd == EVT_RAW_PKT_READY) {
      ...
      dbgtrace(__LINE__, tx_pkt_info.cmd, adbgtraceHIF, K_DBGTRACE_HIF, &idbgtraceHIF);      
    }
...
void    hif_release_inputQ(void) {
    ...
    while(uxQueueMessagesWaiting(q_id_inQ_hif)) {
        ...
        //dbgtrace(__LINE__, (uint8_t)get_datablk_index(pdatablk) & 0xFF, adbgtraceHIF, K_DBGTRACE_HIF, &idbgtraceHIF);
        if(pdatablk) { 
            ...
            configASSERT(pdatablk->dbHeader.numUseCount);
            datablk_mgr_release_generic(pdatablk);
        }
    }
}

#define D2H_NUM_AUDIO_BLOCKS_TO_SEND    (6)
    ...
    if (uxQueueMessagesWaiting(q_id_inQ_hif) >= D2H_NUM_AUDIO_BLOCKS_TO_SEND) {
        ...
        if (xSemaphoreTake(p_hif_channel_info->xSemaphore, 0) == pdTRUE) {
            ...
            configASSERT(xResult != pdFAIL);
              ...
              configASSERT( (block_size+kbytes) <= D2H_MAX_DATA_SIZE ) ; //to make sure buffer size matches
...
extern void enable_stream_VR(void);
    if(timeout_count_bytes >= (2*16000*2))
    {
      ...
      {
        //re-enable only if it is less than a maximum KP count
        if(stream_kp_detect_count < MAX_STREAM_KP_DETECTS) {
          enable_stream_VR();
...
#if 1 //have a built in ~10-12 sec data timeout to stop streaming    
extern void hifcb_cmd_host_process_off(void *pdata, int len);

    //if(timeout_count_bytes >= (12*16000*2))
    if(timeout_count_bytes >= (120*16000*2))
    {
      timeout_count_bytes = 0;

//      printf("===Internal Timeout 12 secs\n");
        //printf("===Internal Timeout 120 secs\n");
      dbg_str("===Internal Timeout 120 secs\n");
      hifcb_cmd_host_process_off(NULL, 0);
