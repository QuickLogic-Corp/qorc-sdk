...
#endif
uint32_t fbinterrupt = INTR_CTRL->FB_INTR;
// detect which one of the FB generators inetrrupted
  if ( fbinterrupt & FB_0_INTR_DETECT)
  {
  ...
  if ( fbinterrupt & FB_1_INTR_DETECT)
  {
    // call that particualr ISR
  ...
  if ( fbinterrupt & FB_2_INTR_DETECT)
  {
    // call that particualr ISR
  ...
  if ( fbinterrupt & FB_3_INTR_DETECT)
  {
    // call that particualr ISR
...
void FB_UnRegisterISR(UINT32_t fbIrq, HAL_FBISRfunction ISRfn)
{
	if (fbIrq< MAX_FB_INTERRUPTS)
	{
		FB_ISR [fbIrq] = NULL;
...
void SensorGpio_Handler(void)
{
	...
	{
        /* This is I2S int from Host to Device*/
		service_i2s_intr_from_host();
