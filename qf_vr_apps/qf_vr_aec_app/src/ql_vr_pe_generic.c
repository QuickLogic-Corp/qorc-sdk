...
#define SAVE_DEBUG_BUF_ENABLE (0) // 1=enable if want to save first 1sec of VR buffer
...
int get_fvRenabled(void)
{
  return (int)fVRenabled; 
}
...
uint32_t vr_dwt_start_count = get_timer();     
     ...
     //Hence during I2S data enabled from Host, disable VR
     if(host_i2s_enabled)
       return;
...
#if 0
vr_cycle_count_buf[vr_cycle_counter++] = get_timer() - vr_dwt_start_count;
if(vr_cycle_counter >= 20)
{
  vr_cycle_counter = 0;
  uint32_t sum = 0;
  for(int i = 0;i <20; i++)
    sum += vr_cycle_count_buf[i];
  sum = (sum*10)/(1*3);
  printf("-vr cyc %d-", sum);     
}
#endif
