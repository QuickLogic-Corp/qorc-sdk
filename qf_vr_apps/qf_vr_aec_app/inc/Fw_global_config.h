...
#define FEATURE_1WIRE_PROTOCOL_DEVICE  (1) //1 = 1-Pin protocol, 0 = 4-pin protocol
...
#define ENABLE_I2S_16KHz_RX_SLAVE (1) //Audio data is received on I2S interface at 48KHz, but decimated by 3 for SDMA

#if (ENABLE_I2S_16KHz_RX_SLAVE == 1)
#define ENABLE_I2S_SLAVE_FB_RX (1)
#endif

#define ENABLE_FB_FIR_DECIMATION_RX (1) //1=decimation by 3 is done in FPGA

#define FEATURE_MULTI_CHANNEL_STREAM_DEVICE  (1) //1 = 2 or 3 channel Data Stream, 0 = 1 channel stream (source circular buffer)

#if (FEATURE_MULTI_CHANNEL_STREAM_DEVICE == 1)
#define NUM_AUDIO_STREAM_CHANNELS  (3) //either 2 or 3 only
#else
#define NUM_AUDIO_STREAM_CHANNELS  (0) //not used for regular stream from Circular Buffer 
#endif

#define AEC_ENABLED  (1)
//see ql_audio_preproc.h for definitions
#define QL_AUDIO_PREPROC_ALGO (15) //QL_AUDIO_PREPROC_CONS_AEC
#if (AEC_ENABLED == 1)
#define AEC_2_0    (1)
#endif
