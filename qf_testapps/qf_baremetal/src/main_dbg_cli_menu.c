...
uint16_t kbTrigger = 0;
static void usb_trigger(const struct cli_cmd_entry *pEntry)
{
    
    (void)pEntry;
        // Add functionality here
    CLI_uint16_getshow( "number of bytes before report", &kbTrigger );
    return;
}

uint16_t kbWrite = 0;
static void usb_write(const struct cli_cmd_entry *pEntry)
{
...
const struct cli_cmd_entry qf_usb[] =
{
    CLI_CMD_SIMPLE( "write", usb_write, "write data over USB serial" ),
	CLI_CMD_SIMPLE( "trigger", usb_trigger, "wbytes before FIFO report" ),
    CLI_CMD_TERMINATE()
};
...
const struct cli_cmd_entry my_main_menu[] = {
	CLI_CMD_SUBMENU( "usb", qf_usb, "QuickFeather usb speed test" ),
